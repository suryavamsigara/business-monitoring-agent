  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center bg-slate-900/50 rounded-xl border border-slate-800">
        ...
        <p className="text-slate-400 text-sm">Loading business alerts...</p>
      </div>
    );
  }
  ...
  if (!alerts || alerts.length === 0) {
    return (
      <div className="p-12 text-center bg-slate-900/40 rounded-xl border border-slate-800">
        ...
        <h4 className="text-base font-semibold text-slate-200 mb-1">No Active Alerts Found</h4>
      ...
      <table className="w-full text-left text-xs border-collapse">
        <thead>
          <tr className="border-b border-slate-800 bg-slate-950/80 text-slate-400 font-semibold tracking-wider uppercase text-[11px]">
            ...
            <th className="py-3.5 px-4 text-right">Impact / Exposure</th>
          ...
          {alerts.map((alert) => (
            ...
            >
              ...
              <td className="py-3 px-4 min-w-[220px]">
                <Link
                  to={`/alerts/${alert.id}`}
                  className="block font-semibold text-slate-100 group-hover:text-amber-300 transition-colors hover:underline"
                >
                  {alert.title}
                </Link>
              ...
              {/* KPI */}
              <td className="py-3 px-4 whitespace-nowrap">
                <span className="bg-slate-800/90 text-slate-300 font-mono text-[11px] px-2 py-0.5 rounded border border-slate-700/50">
              ...
              <td className="py-3 px-4 whitespace-nowrap font-medium text-slate-300">
                {alert.entity_name || "Business-wide"}
              </td>
              ...
              <td className="py-3 px-4 text-right whitespace-nowrap">
                <div className="inline-flex items-center gap-1.5">
                  <select
                    ...
                    className="bg-slate-950 text-slate-300 border border-slate-700/80 rounded px-2 py-1 text-[11px] focus:outline-none focus:border-amber-500 transition-colors"
                  >
                  ...
                  <Link
                    to={`/alerts/${alert.id}`}
                    className="p-1 rounded hover:bg-slate-800 text-slate-400 hover:text-amber-300 transition-colors inline-flex items-center"
                    title="View Investigation Detail"
                  >
